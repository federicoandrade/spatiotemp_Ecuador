...
#There was a conflict with select (probably with raster)

proy_pob2010_2020 <- read.csv("Data/proyeccion_cantonal_total_2010-2020.csv", sep = " ", header= TRUE,  stringsAsFactors=FALSE)
              
###### Join Canton Ubi. Si CantRes missing, then CarUbi used instead. 
...
eg2011_2019_extVar <- eg2011_2019_extVar0  %>% left_join(y = CantCentr_df, by = c("cant_res" = "DPA_CANTON")) %>% 
  ...
  mutate(CordXres = coalesce(CordXres, CordXubi),
         CordYres = coalesce(CordYres, CordYubi, Nom_CantRes = coalesce(Nom_CantRes, NomCantUbi)))
